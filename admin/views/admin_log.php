...
<div class=containertitle><b><?php echo $pwd; ?></b>
    <?php if(!isset($_GET['active_del']) && !isset($_GET['active_up']) && !isset($_GET['active_down']) && !isset($_GET['error_a']) && !isset($_GET['error_b']) && !isset($_GET['active_post']) && !isset($_GET['active_move']) && !isset($_GET['active_change_author']) && !isset($_GET['active_hide']) && !isset($_GET['active_savedraft']) && !isset($_GET['active_savelog']) && !isset($_GET['active_ck']) && !isset($_GET['active_unck']) ):?>
        <span id="msg_2">
    ...
    <?php if(isset($_GET['active_up'])):?><div class="alert alert-success">置顶成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_down'])):?><div class="alert alert-success">取消置顶成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['error_a'])):?><div class="alert alert-warning">请选择要处理的文章<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    ...
    <?php if(isset($_GET['active_post'])):?><div class="alert alert-success">发布成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_move'])):?><div class="alert alert-success">移动成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_change_author'])):?><div class="alert alert-success">更改作者成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_hide'])):?><div class="alert alert-success">转入草稿箱成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_savedraft'])):?><div class="alert alert-success">草稿保存成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_savelog'])):?><div class="alert alert-success">保存成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_ck'])):?><div class="alert alert-success">文章审核成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
    <?php if(isset($_GET['active_unck'])):?><div class="alert alert-success">文章驳回成功<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div><?php endif;?>
</div>
<div class=line></div>
<div class="filters">
<div id="f_title">
	<div style="float:left; margin-top:8px;">
		<span <?php echo !$sid && !$tagId && !$uid && !$keyword ? "class=\"filter\"" : ''; ?>><a href="./admin_log.php?<?php echo $isdraft; ?>" class="btn btn-default btn-sm">全部</a></span>
        <span id="f_t_sort">
            <select name="bysort" id="bysort" onChange="selectSort(this);" style="width:110px;" class="btn btn-default btn-sm">
        ...
        <span id="f_t_user">
            <select name="byuser" id="byuser" onChange="selectUser(this);" style="width:110px;" class="btn btn-default btn-sm">
        ...
        <?php endif;?>
        <span id="f_t_tag"><a href="javascript:void(0);" class="btn btn-default btn-sm">按标签查看</a></span>
	</div>
	<div style="float:right;">
		<form action="admin_log.php" method="get">
		<input type="text" id="input_s" name="keyword" style="margin-top: 9px;" class="form-control input-sm" placeholder="搜索...">
		<?php if($pid):?>
...
<form action="admin_log.php?action=operate_log" method="post" name="form_log" id="form_log" role="form">
  ...
  <thead>
      <tr>
		...
		<th width="100">作者</th>
        <th width="146">分类</th>
        <th width="130"><a href="./admin_log.php?sortDate=<?php echo $sortDate.$sorturl; ?>">时间</a></th>
		<th width="39" class="tdcenter"><a href="./admin_log.php?sortComm=<?php echo $sortComm.$sorturl; ?>">评论</a></th>
		<th width="59" class="tdcenter"><a href="./admin_log.php?sortView=<?php echo $sortView.$sorturl; ?>">阅读</a></th>
      </tr>
	</thead>
 	<tbody>
	<?php
...
<!--	<a href="javascript:void(0);" id="select_all">全选</a> -->
    选中项：
    <a href="javascript:logact('del');" class="care btn btn-default btn-sm">删除</a>
	<?php if($pid == 'draft'): ?>
	<a href="javascript:logact('pub');" class="btn btn-default btn-sm">发布</a>
	<?php else: ?>
	<a href="javascript:logact('hide');" class="btn btn-default btn-sm">放入草稿箱</a>

	<?php if (ROLE == ROLE_ADMIN):?>
    <select name="top" id="top" onChange="changeTop(this);" style="width:90px;" class="btn btn-default btn-sm">
	...
	<select name="sort" id="sort" onChange="changeSort(this);" style="width:110px;" class="btn btn-default btn-sm">
