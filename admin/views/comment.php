...
<form action="comment.php?action=admin_all_coms" method="post" name="form_com" id="form_com">
  <table width="100%" id="adm_comment_list" class="table table-striped table-hover">
      ...
      <tr>
        ...
        <th width="358"><b>内容</b></th>
		<th width="150"><b>评论者</b></th>
        <th width="150"><b>IP</b></th>
	...
	?>
     <tr>
		...
		<td><?php echo $poster;?> <?php echo $mail;?></td>
        <td><?php echo $ip;?> <?php if (ROLE == ROLE_ADMIN): ?><a href="javascript: em_confirm('<?php echo $value['ip']; ?>', 'commentbyip', '<?php echo LoginAuth::genToken(); ?>');" title="删除来自该IP的所有评论" style="font-size: 20px;position: relative;top: -5px;left: 10px;">&times;</a><?php endif;?></td>
